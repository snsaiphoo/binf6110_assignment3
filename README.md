...
Taxonomic abundance was examined at the family, genus, and species levels to provide both a broad and more detailed view of the microbial community. At the family level, as shown in Figure 2, both vegan and omnivore samples are mainly dominated by _Bacteroidaceae_, _Lachnospiraceae_, _Oscillospiraceae_, and _Prevotellaceae_, although their proportions vary between samples. Vegan samples show higher levels of _Lachnospiraceae_, while omnivore samples show higher levels of _Bacteroidaceae_. This points to differences in how these communities function. For example, Lachnospiraceae are involved in breaking down dietary fiber and producing short-chain fatty acids [19], which suggests that vegan diets may support more fiber-related metabolism.

Figure 3 shows a similar overall pattern, with Bacteroides, Faecalibacterium, Blautia, and Alistipes standing out as the main genera. These genera appear across all samples, but their levels differ. _Bacteroides_ remains consistently high, _Blautia_ is higher in several vegan samples, and _Faecalibacterium_ and Alistipes are more prominent in many omnivore samples. _Blautia_ is known to play a role in carbohydrate metabolism [20], which again supports the idea of increased fiber use in vegan diets. _Faecalibacterium_ is often linked to gut health [21], while _Alistipes_ has been associated with inflammatory responses in some cases [22]. These patterns suggest that vegan samples may be more associated with fiber-related metabolic activity, while omnivore samples may reflect differences linked to protein and fat intake.

At the species level, Figure 4 shows more variation between samples, which is expected at a more detailed level. _Prevotella_ _copri_ is more abundant in several vegan samples and is known to be associated with carbohydrate metabolism and fiber-rich diets [1, 23]. This supports the idea that plant-based diets select for microbes that can break down complex carbohydrates. Other species, such as _Faecalibacterium_ _prausnitzii_ and _Bacteroides_, are found in both groups but vary between samples. Overall, the higher variability at this level shows that individual differences are important, and diet-related patterns may be clearer when looking more closely at specific taxa.
...
Alpha diversity measures how diverse each individual sample is, including both the number of taxa and how evenly they are distributed. Beta diversity compares how different the microbial communities are between samples.

The alpha diversity results in Figure 6 show that omnivore samples tend to have more taxa overall than vegan samples. In other words, omnivore microbiomes include more species, but the balance between those species is similar across both groups. This suggests that the overall structure of the microbial community is not very different between diets, even if the number of taxa changes slightly.

Beta diversity, shown in Figure 7, shows some separation between vegan and omnivore samples, but there is still clear overlap. This means that diet may have some effect on microbial composition, but it is not strong enough to clearly separate the groups. Individual variation likely plays a larger role, and the small sample size may also limit the ability to detect consistent differences. Overall, these results suggest that differences between vegan and omnivore microbiomes are present, but not strong based on diversity measures alone.
...
This highlights an important limitation of differential abundance analysis. Smaller taxa can show large relative changes and appear significant, even when the main microbial groups stay the same [24]. This is also affected by the compositional nature of microbiome data, where changes in one group can shift the relative abundance of others [24, 25]. Because of this, statistically significant results do not always reflect major biological changes in the overall microbiome.

At the genus level, Figure 9 shows that no genera were statistically significant. This matches what is seen in Figure 3, where dominant genera such as Bacteroides, Blautia, and Faecalibacterium are present in both groups without clear separation. Even though some genera appear higher in vegan samples, these differences are not statistically significant, suggesting that they are likely due to variation between samples rather than true diet effects.

Overall, while this analysis aimed to identify diet-related differences in the gut microbiome, similar to those reported by De Filippis et al. [1], the results here show only limited evidence for strong patterns. Previous studies have linked diet to changes in microbial composition and function, including the role of Prevotella copri in carbohydrate metabolism [1]. However, those patterns were not observed in this analysis; this does not mean that diet doesn’t influence the microbiome, but the effects seen here are most likely due to a small sample size.
